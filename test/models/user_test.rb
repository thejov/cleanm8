...
class UserTest < ActiveSupport::TestCase
  ...
  test 'should not save user without arguments' do
    a_user = User.new()
    assert_not a_user.save, 'Saved user without any given arguments!'
  end

  test 'should not save user without name' do
    a_user = create_user.without_name.build
    assert_not a_user.save, 'Saved user without any given name!'
  end

  test 'should not save user without email' do
    a_user = create_user.without_email.build
    assert_not a_user.save, 'Saved user without any given email!'
  end

  test 'should not save user without password' do
    a_user = create_user.without_password.build
    assert_not a_user.save, 'Saved user without any given password!'
  end
  ...
  test 'should not save user with same email address' do
    ...
    assert_not another_user.save, 'Saved user with identical email address to existing other user!'
  end
